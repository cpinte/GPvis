...
def plot_visibilities(x: np.ndarray, y_real: np.ndarray, y_imag: np.ndarray,
                     ...
                     yerr_real: np.ndarray, yerr_imag: np.ndarray,
    ...
    Args:
        ...
        yerr_real: Error bars for real component
        yerr_imag: Error bars for imaginary component
    ...
    # Plot real component
    ax1.errorbar(x, y_real, yerr=yerr_real, fmt='o', color='blue',
                markersize=2, alpha=0.5, label='Original', capsize=2)
    ...
    ax2.errorbar(x, y_imag, yerr=yerr_imag, fmt='o', color='blue',
                markersize=2, alpha=0.5, label='Original', capsize=2)
...
def denoise_visibilities(target: str, plot: bool = False) -> None:
    ...
    search_name = target + "*[0-9].vis.npz"
    ...
    for npz_file in npz_files:
        denoise_npz(npz_file,plot=plot,output_dir=output_dir)




def denoise_npz(npz_file, plot=False, output_dir="./plots"):
    ...
    try:
        ...
        print(f"Error processing {npz}: {str(e)}")
        return
    ...
    denoised_Vis = np.zeros_like(Vis)
    ...
    #list_ant1 = [list_ant1[0]]
    #list_ant2 = [list_ant2[0]]
    #list_spwid= [list_spwid[0]]
    #list_ant1 = [10]
    #list_spwid = [0]
        ...
        for a1 in list_ant1:
            for a2 in list_ant2:
                ...
                for spw in list_spwid:
                    ...
                    # Check that data is already sorted in time just time
                    #print("-------------------")
                    #print("is_sorted", a1, a2, spw, is_sorted(x))
                    ...
                    for i, y in enumerate([V.real, V.imag]):
                        ...
                        while error_encountered and err_counter < MAX_RETRIES:
                            ...
                            try:
                                ...
                                #print(a1, a2, spw, "TEST", np.std(y), log_c)
                    ...
                    if real_vis is not None and imag_vis is not None:
                        ...
                        denoised_Vis[indices] = denoised_vis
                    else:
    ...
    os.system("rm -rf "+new_name)
    print("Writing "+new_name)
    np.savez(new_name, u=u, v=v, Vis=denoised_Vis, Wgt=Wgt, ant1=ant1, ant2=ant2, time=time, spwid=spwid)
